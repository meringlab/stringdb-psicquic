/**
 * Copyright 2014 EBI, University of Zürich, SIB, and others.
 *
...
public class StringdbSolrBasedPsicquicRestService implements PsicquicRestService {
    ...
    @Override
    public Object getByInteractor(String interactorAc, @DefaultValue("") String db, @DefaultValue("tab25") String format, @DefaultValue("0") String firstResult, @DefaultValue("2147483647") String maxResults) throws PsicquicServiceException, NotSupportedMethodException, NotSupportedTypeException {
    ...
    @Override
    public Object getByInteraction(String interactionAc, @DefaultValue("") String db, @DefaultValue("tab25") String format, @DefaultValue("0") String firstResult, @DefaultValue("2147483647") String maxResults) throws PsicquicServiceException, NotSupportedMethodException, NotSupportedTypeException {
        String query = SolrFieldName.interaction_id.toString() + ":" + createQueryValue(interactionAc, db);
        return getByQuery(query, format, firstResult, maxResults);
    }

    @Override
    public Object getByQuery(String query, @DefaultValue("tab25") String format, @DefaultValue("0") String firstResultStr, @DefaultValue("2147483647") String maxResultsStr) throws PsicquicServiceException, NotSupportedMethodException, NotSupportedTypeException {
        ...
        try {
            if (RETURN_TYPE_XML25.equalsIgnoreCase(format)) {
                ...
                final EntrySet entrySet = PsicquicConverterUtils.extractJaxbEntrySetFromPsicquicResults(psicquicResults, query, maxResults, XgmmlStreamingOutput.SolrBasedPsicquicService_BLOCKSIZE_MAX);
                long count = psicquicResults.getNumberResults();

                return prepareResponse(Response.status(200).type(MediaType.APPLICATION_XML), new GenericEntity<EntrySet>(entrySet) {
                }, count).build();
                ...
                PsicquicSearchResults psicquicResults = psicquicSolrServer.search(query, firstResult, Math.min(maxResults, XgmmlStreamingOutput.SolrBasedPsicquicService_BLOCKSIZE_MAX), PsicquicSolrServer.RETURN_TYPE_MITAB27, config.getQueryFilter());
                ...
                if (RETURN_TYPE_COUNT.equalsIgnoreCase(format)) {
                    ...
                    return prepareResponse(Response.status(200).type(MediaType.TEXT_PLAIN), new GenericEntity<Long>(psicquicResults.getNumberResults()) {
                    }, psicquicResults.getNumberResults()).build();
                    ...
                    return prepareResponse(Response.status(200).type(MediaType.TEXT_PLAIN), new GenericEntity<PsicquicStreamingOutput>(psicquicStreaming) {
                            ...
                            psicquicResults.getNumberResults()
                    ).build();
